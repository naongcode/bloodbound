...
│   ├── AuthScene.js           # 로그인/회원가입 UI
│   ├── JobSelectScene.js      # 직업 선택 화면
│   ├── LobbyScene.js          # 멀티플레이 로비
│   ├── WaitingScene.js        # 파티 대기실
│   ├── GameScene.js           # 솔로 메인 게임 루프, 이벤트 허브
│   ├── DungeonScene.js        # Co-op 던전 씬
│   ├── GuildScene.js          # 길드 관리 화면
...
│   ├── GuildSystem.js         # 길드 생성/가입/관리
│   ├── NetworkManager.js      # 멀티플레이 네트워크 (Supabase Realtime)
│   ├── AuthManager.js         # 인증 상태 관리
│   └── SaveSystem.js          # 세이브/로드 (Supabase DB)
└── data/
